...
[3단계] 게시물 로드 및 스크립트 실행 확인
        ...
        │
        ▼
[4단계] 고도화 공격 (세션 탈취 등 시스템/사용자 정보 해킹)
        (타 사용자 브라우저 접속 시 쿠키 정보를 공격자 서버로 몰래 빼돌리는 스크립트 저장)
```
...
- **스크립트 영구 저장(DB)**:
  ...
  ```
- **파급 효과(시스템 정보/사용자 영향)**: 일반 사용자 인증 세션(Session) 탈취, 브라우저 화면 강제 조작(Deface), 다른 사용자의 권한을 도용하여 추가 악성 행위 유도 가능.
